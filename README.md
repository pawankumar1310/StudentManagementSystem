...
``` 
 ...
 ### About Github Repository related to Student Management:

 1. SampleDemo : In this repo, i have just created api for crud operation using Database first approach Entity Framework. Run it through swagger.Created Database (StudentDbDemo) Table name : Student. Import flat file(Student-Original.csv). Made Student_Id as primary that's it.

 2. RestAPISample : In this repo, I have created api like SampleDemo repo. Only difference is that  in this repo, I have created another database (StudentDB) Table name : Student which has validation like below using Database First Approach Entity Framework:
 ```bash
...
```
3. StudentManagementSystem : In this repo, I have two branch,
   - main : In this branch all code like SampleDemo repo.
           Database(StudentDbDemo) and Table(Student).
   - DbFirstApproach : In  this branch I have implemented the same like RestAPISample repo. All the validation.
   Database(StudentDB) and Table(Student).

4. RestAPIFrontendInterface : This is only  front end part of the RestAPISample repo. RestAPISample is the backend api. Which integrated with RestAPIFrontendInterface.It contains three folder :
      - HTML : Which contains all html file
      - CSS : Which contains all css file
      - Javascript : Which will intergrate with api url.

5. WebAPIADO.Net : This repo is only responsible for creating api for crud operation of student using ADO.net without using Stored procedure. In the code i have written all the query for crud operation of a student. It will connect database (StudentDB).

6. BlazorWebAPIStoredProcedure : This is the important repo in which  I am using Stored Procedure to do  CRUD operation on database of a Student. It contains all the feature like, loggin,validation, modals, softdelete, database truncation, data export, data seeder and coding standards.This api will integrated with Blazor application(BlazorWebAPI repo).
   - Database : StudentDB
   - Table : Student

7. BlazorWebAPI : In this repo , I have integrated BlazorWebAPIStoredProcedure repo's api using Blazor  WebAssembly Hosted mode.